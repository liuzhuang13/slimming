...
function Trainer:gradientBN(weight, gradWeight)
   -- print(self.opt.loss)
   -- print('l1')
   -- print(self.opt.loss=='l1')
   local S = self.opt.S
   local M = 1

   -- local cond = (weight:gt(0) + weight:lt(M)):gt(1.5):float():cuda()
   -- local co = (2*cond-1)

   -- local add =  S*co:cmul(M - 2*weight)
   -- gradWeight:add(add)




   --1122 change additional loss function
   -- if self.opt.loss == '1124' then
   --    local cond = (weight:gt(0) + weight:lt(M/2)):gt(1.5):float():cuda()
   --    local co = cond
   --    local add = S*co:cmul(M - 2*weight)
   --    gradWeight:add(add)

   -- elseif self.opt.loss == 'l1' then
   --    -- print('l1')
      local add = S*torch.sign(weight)
      gradWeight:add(add)
       -- print('gradient')
   -- elseif self.opt.loss == 'l2' then
   --    local add = S*2*weight
   --    gradWeight:add(add)
   -- end
end

function Trainer:bn()
   -- local fcsize = model:get(model:size()).weight:size()[2]
   local name = 'cudnn.SpatialBatchNormalization'
   for k, v in pairs(self.model:findModules(name)) do
      -- if v.weight:size()[1] < fcsize then
         self:gradientBN(v.weight, v.gradWeight)
      -- end
         -- print
   end

   local name = 'cudnn.BatchNormalization'
   for k, v in pairs(self.model:findModules(name)) do
      -- if v.weight:size()[1] < fcsize then
         self:gradientBN(v.weight, v.gradWeight)
      -- end
